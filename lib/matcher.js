...
export function matchStream(stream, matches, aliasesByTeam) {
   ...
   *
   * Исключение — календарь Матч ТВ: там заголовки структурированные, обе
   * команды присутствуют всегда, поэтому совпадение только одной команды
   * гарантированно означает ДРУГОЙ матч того же клуба («Факел — Балтика»
   * нельзя привязывать к «Балтика — Локомотив»).
   */
  const strictPairs = stream.platform === 'matchtv';
  return results
    .filter((r) => (r.matched === 2 && r.score >= 0.78)
      || (!strictPairs && r.matched === 1 && r.league && r.score >= 0.58))
